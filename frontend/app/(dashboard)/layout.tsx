...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="flex h-screen w-full overflow-hidden bg-[#f5f5f5]">
    {open && <button aria-label="Close navigation" className="fixed inset-0 bg-slate-950/40 z-30 lg:hidden" onClick={() => setOpen(false)} />}
    <aside id="workspace-navigation" className={`fixed inset-y-0 left-0 z-40 w-[230px] overflow-hidden bg-[#1a2744] flex flex-col transition-transform lg:static lg:translate-x-0 lg:shrink-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="px-5 py-4 flex items-center gap-3 border-b border-white/5 shrink-0"><span className="bg-white/10 p-2 rounded-xl text-white"><Shield size={20} /></span><div><p className="font-bold text-white text-base">FeedbackWell</p><p className="text-[9px] text-white/40">Secure document collection</p></div><button aria-label="Close menu" onClick={() => setOpen(false)} className="text-white/60 lg:hidden ml-auto"><X size={18} /></button></div>
      <div className="mx-4 mt-4 mb-3 p-3 rounded-xl bg-white/5 border border-white/[0.06] shrink-0"><p className="text-sm text-white font-medium truncate">{organization?.name || 'FeedbackWell'}</p><p className="text-[10px] text-white/40 mt-0.5">Organization workspace</p></div>
      <nav aria-label="Workspace" className="px-3 pb-3 shrink-0">
        <p className="text-[10px] text-white/35 font-semibold tracking-widest px-3 mb-2">WORKSPACE</p>
        {navigation.map(({ href, label, icon: Icon }) => <Link key={href} href={href} onClick={() => setOpen(false)} aria-current={pathname === href || pathname.startsWith(href + '/') ? 'page' : undefined} className={`flex gap-3 items-center px-3 py-2.5 rounded-xl text-xs font-medium mb-1 transition-colors ${pathname === href || pathname.startsWith(href + '/') ? 'bg-white/10 text-white' : 'text-white/55 hover:bg-white/5 hover:text-white'}`}><Icon size={16} />{label}</Link>)}
        {canAudit && <Link href="/audit" onClick={() => setOpen(false)} className={`flex gap-3 items-center px-3 py-2.5 rounded-xl text-xs font-medium transition-colors ${pathname.startsWith('/audit') ? 'bg-white/10 text-white' : 'text-white/55 hover:bg-white/5 hover:text-white'}`}><History size={16} />Audit history</Link>}
      </nav>
      <div className="p-4 pt-3 border-t border-white/5 mt-auto shrink-0">
    ...
    <div className="flex-1 min-w-0 flex flex-col overflow-hidden"><header className="h-16 bg-white border-b border-slate-100 px-4 sm:px-6 flex gap-3 items-center shrink-0"><button aria-label="Open navigation" aria-controls="workspace-navigation" aria-expanded={open} onClick={() => setOpen(true)} className="lg:hidden text-slate-600 p-2"><Menu size={21} /></button><p className="hidden text-sm font-semibold tracking-tight text-[#2d4a7a] sm:block">Lender Dashboard</p><div className="ml-auto flex items-center gap-3"><div className="text-right"><p className="text-xs font-semibold text-slate-800">{user.firstName} {user.lastName}</p><p className="text-[10px] text-slate-400 capitalize mt-0.5">{user.role.replaceAll('_', ' ')}</p></div><span className="w-9 h-9 rounded-full bg-[#2d4a7a] flex items-center justify-center text-white text-xs font-semibold">{user.firstName[0]}{user.lastName[0]}</span></div></header><main className="flex-1 min-h-0 min-w-0 overflow-y-auto">{children}</main></div>
  </div>;
}
