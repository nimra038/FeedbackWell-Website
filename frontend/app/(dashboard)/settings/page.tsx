...
function SettingsForm({ organization }: { organization: Organization }) {
  ...
  const [form,setForm]=useState({name:organization.name,brand_color:organization.brand_color||'#2d4a7a',timezone:organization.timezone,country:organization.country,website:organization.website||'',logo:organization.logo||''});const [notice,setNotice]=useState('');const [error,setError]=useState('');const [busy,setBusy]=useState(false);
  ...
  const fields = [
    ...
    { key: 'website' as const, label: 'Website', icon: Globe2, type: 'url' },
    { key: 'logo' as const, label: 'Logo URL', icon: Building2, type: 'url' },
  ];
